...
$(document).ready(function () {
  ...
  function downloadCanvasAsPNG(hdPixiApp, filename, $container) {
    ...
    hdPixiApp.destroy();
    var $link = $("<a download=\"".concat(filename, "\">\u9EDE\u6B64\u4E0B\u8F09\u5716\u7247</a>")); /// convert canvas content to data-uri for link. When download
    /// attribute is set the content pointed to by link will be
    /// pushed as "download" in HTML5 capable browsers

    $link.attr('href', image.replace("image/png", "image/octet-stream")); /// create a "fake" click-event to trigger the download
